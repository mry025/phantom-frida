...
def create_vala_wrapper(frida_dir: Path, arch: str):
    """Create valac wrapper script for cross-compilation.
    
    Frida's meson build expects architecture-specific valac wrappers
    like 'frida-android-arm64-valac', but these aren't generated by SDK.
    We create simple wrappers that call the system valac.
    
    This must be called BEFORE meson is invoked, as the cross-compile file
    will reference the wrapper path.
    """
    build_dir = frida_dir / "build"
    if not build_dir.exists():
        build_dir.mkdir(parents=True, exist_ok=True)
    
    # Map arch to valac wrapper name
    wrapper_name = f"frida-{arch}-valac"
    wrapper_path = build_dir / wrapper_name
    
    # Find valac path first
    import subprocess
    valac_path = None
    try:
        # Try to find valac
        for path in ["/usr/bin/valac", "/usr/local/bin/valac"]:
            if os.path.exists(path):
                result = subprocess.run([path, "--version"], capture_output=True, text=True, timeout=5)
                if result.returncode == 0:
                    valac_path = path
                    log(f"  Found working valac at: {valac_path}", "INFO")
                    break
        
        if not valac_path:
            # Try using PATH
            result = subprocess.run(["which", "valac"], capture_output=True, text=True, timeout=5)
            if result.returncode == 0 and result.stdout.strip():
                valac_path = result.stdout.strip()
                log(f"  Found valac in PATH: {valac_path}", "INFO")
    except Exception as e:
        log(f"  Warning: Could not find valac: {e}", "WARN")
    
    if not valac_path:
        valac_path = "valac"  # Fallback to PATH lookup
        log(f"  Will use 'valac' from PATH", "WARN")
    
    # Create simple wrapper that just calls valac
    # No extra logic, just direct execution
...
"""
    ...
    log(f"  Wrapper permissions: {oct(file_stat.st_mode)}, executable: {is_executable}", "INFO")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log(f"  Valac wrapper failed (exit {result.returncode}): {result.stderr.strip()}", "ERROR")
            log(f"  This may cause meson to fail!", "ERROR")
    except Exception as e:
        log(f"  Could not verify valac wrapper: {e}", "ERROR")
...
def ensure_cross_file_valac(frida_dir: Path, arch: str, force: bool = False):
    ...
    Args:
        frida_dir: Frida source directory
        arch: Architecture (e.g., "android-arm64")
        force: If True, always rewrite the valac config even if it exists
    """
    build_dir = frida_dir / "build"
    cross_file = build_dir / f"frida-{arch}.txt"
    ...
    # Check if valac is already in the file
    has_valac = 'valac' in content
    
    if force or not has_valac:
        # Add valac to [binaries] section
        if '[binaries]' in content:
            if has_valac:
                # Remove existing valac line and add new one
                lines = content.split('\n')
...
def build_frida(frida_dir: Path, ndk_path: Path, arch: str = None):
    ...
    # For Makefile builds, we need to ensure cross file has valac
    # The cross file is generated by Makefile during build, so we need to 
    # modify it after make starts but before meson is called for frida-core
    if arch:
        # First attempt: try to modify cross file before make starts
        # (in case it was already generated by configure step)
        ensure_cross_file_valac(frida_dir, arch, force=True)
    ...
    if meson_build_dir.exists():
        ...
        if has_makefile:
            ...
            # We need to create a wrapper that modifies the cross file just before meson runs
            # For now, let's try a different approach: create the cross file manually
            build_dir = frida_dir / "build"
            if not build_dir.exists():
                build_dir.mkdir(parents=True, exist_ok=True)
            
            # Create valac wrappers BEFORE make starts
            # This ensures they exist when cross file is generated
            log("  Creating valac wrappers before make...", "INFO")
            create_vala_wrapper(frida_dir, arch)
            
            # Create a minimal cross file if it doesn't exist
            cross_file = build_dir / f"frida-{arch}.txt"
            if not cross_file.exists():
                log(f"  Creating minimal cross file: {cross_file}", "INFO")
                # Map arch to meson cpu family
                arch_to_cpu = {
                    "android-arm64": ("aarch64", "arm64-v8a"),
                    "android-arm": ("arm", "armeabi-v7a"),
                    "android-x86_64": ("x86_64", "x86_64"),
                    "android-x86": ("x86", "x86"),
                }
...
"""
                ...
                # Ensure existing cross file has valac
                log(f"  Cross file already exists, checking valac config", "INFO")
                ensure_cross_file_valac(frida_dir, arch, force=True)
            
            # Create a modified make command that ensures valac is in cross file
            # We'll use a subshell to modify the cross file right before meson runs
...
# Wrapper script to:
# 1. Ensure valac is in cross file before meson runs (checks before EVERY meson call)
# 2. Fix ninja target names for Frida 15.x

# Function to fix cross file - called before EVERY meson invocation
fix_cross_file() {{
    local cross_files=("{str(build_dir / f'frida-android-arm.txt')}" "{str(build_dir / f'frida-android-arm64.txt')}" "{str(build_dir / f'frida-android-x86.txt')}" "{str(build_dir / f'frida-android-x86_64.txt')}")
    
    for cross_file in "${{cross_files[@]}}"; do
        if [ -f "$cross_file" ]; then
            # Extract arch from filename
            arch=$(basename "$cross_file" .txt | sed 's/frida-//')
            wrapper_path="{str(build_dir)}/frida-$arch-valac"
            
            if [ -f "$wrapper_path" ]; then
                if ! grep -q "valac" "$cross_file"; then
                    echo "[FIX] Adding valac to cross file: $cross_file (arch=$arch)"
                    # Add valac after [binaries] section
...
# Override meson command to fix cross file before each invocation
meson() {{
    # Fix cross files before calling meson
    fix_cross_file
    # Call the real meson
    command meson "$@"
}}
...
exec make "$@"
"""
            make_wrapper.write_text(make_wrapper_content)
            make_wrapper.chmod(0o755)
            log(f"  Created make wrapper: {make_wrapper}", "INFO")
            
            # Use the wrapper instead of direct make
            run(

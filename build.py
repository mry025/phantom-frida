...
def apply_targeted_patches(frida_dir: Path, custom_name: str, frida_major: int):
    ...
    # NOTE: agent-glue.c and gadget-glue.c include headers generated by Vala.
    # Vala uses the library name as header prefix, so we must update the includes.
    targets = {
        ...
        "gadget_glue": core_dir / "lib" / "gadget" / "gadget-glue.c",
        "agent_glue": core_dir / "lib" / "agent" / "agent-glue.c",
...
def build_frida(frida_dir: Path, arch: str, ndk_path: Path):
    """
    Build Frida for the specified architecture.
    Frida uses direct make targets, not ./configure.
    For Android: make core-android-arm64, core-android-arm, etc.
    """
    cpus = os.cpu_count() or 4
    log(f"Building for {arch} ({cpus} threads)...", "STEP")
    # Frida uses make targets like: core-android-arm64, gum-android-arm64
    # We need core target for server/agent/gadget
    run(
        f"make core-{arch} -j{cpus}",
        cwd=str(frida_dir),

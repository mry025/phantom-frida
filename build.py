...
# This wrapper is generated by phantom-frida build script

# Debug: log execution (uncomment for debugging)
# echo "valac wrapper called with: $@" >&2

# Try to find valac
if command -v valac >/dev/null 2>&1; then
    exec valac "$@"
elif [ -x /usr/bin/valac ]; then
    exec /usr/bin/valac "$@"
else
    echo "ERROR: valac not found" >&2
    exit 1
fi
"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log(f"  Valac wrapper failed (exit {result.returncode}): {result.stderr.strip()}", "WARN")
            # Try to find valac directly
            for valac_path in ["/usr/bin/valac", "/usr/local/bin/valac"]:
                if os.path.exists(valac_path):
                    try:
                        ...
                        if result2.returncode == 0:
                            log(f"  Found working valac at {valac_path}: {result2.stdout.strip()}", "INFO")
...
"""
                            ...
                            log(f"  Updated wrapper to use absolute path: {valac_path}", "INFO")
                            break
                    except Exception as e2:
                        log(f"  Valac at {valac_path} also failed: {e2}", "WARN")
            else:
                log(f"  No working valac found in standard paths", "ERROR")
    except Exception as e:

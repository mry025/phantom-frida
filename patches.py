...
def get_targeted_patches(name: str, cap_name: str, target: str) -> list[tuple[str, str]]:
    ...
    if target == "server_meson":
        ...
        return [
            ("shared_library('frida-gadget',", f"shared_library('{name}-gadget',"),
            ("'frida-gadget-modulated'", f"'{name}-gadget-modulated'"),
            ("'frida-gadget',", f"'{name}-gadget',"),
        ...
        return [
            ("shared_library('frida-agent',", f"shared_library('{name}-agent',"),
            ("'frida-agent-modulated'", f"'{name}-agent-modulated'"),
            ("'frida-agent',", f"'{name}-agent',"),
        ]

    elif target == "agent_glue":
        # subprojects/frida-core/lib/agent/agent-glue.c
        # Update include to match the header generated by Vala (uses library name)
        return [
            ('#include "frida-agent.h"', f'#include "{name}-agent.h"'),
        ]

    elif target == "gadget_glue":
        # subprojects/frida-core/lib/gadget/gadget-glue.c
        # Update include to match the header generated by Vala (uses library name)
        return [
            ('#include "frida-gadget.h"', f'#include "{name}-gadget.h"'),
        ]

...
def get_targeted_patches(name: str, cap_name: str, target: str) -> list[tuple[str, str]]:
    ...
    if target == "server_meson":
        ...
        #
        # IMPORTANT: Must add vala_header to preserve header filename
        # because gadget-glue.c includes 'frida-gadget.h'
        return [
            # Raw gadget library: add vala_header to preserve header name
            ("shared_library('frida-gadget', gadget_sources,",
             f"shared_library('{name}-gadget', gadget_sources,\n  vala_header: 'frida-gadget.h',"),
            # custom_target names
            ("'frida-gadget-modulated'", f"'{name}-gadget-modulated'"),
            ("'frida-gadget',", f"'{name}-gadget',"),
            # dylib identity (macOS/iOS)
        ...
        #
        # IMPORTANT: We must preserve the vala_header name as 'frida-agent.h'
        # because agent-glue.c includes it by that name. The original meson.build
        # does NOT specify vala_header, so Vala uses the library name as header prefix.
        # We add explicit vala_header to preserve the header filename.
        return [
            # Raw agent library: add vala_header to preserve header name
            # Original: shared_library('frida-agent', agent_sources,
            # Patched: shared_library('ajeossida-agent', agent_sources, vala_header: 'frida-agent.h',
            ("shared_library('frida-agent', agent_sources,",
             f"shared_library('{name}-agent', agent_sources,\n  vala_header: 'frida-agent.h',"),
            # custom_target names
